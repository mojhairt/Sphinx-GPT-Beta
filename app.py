...
app = FastAPI(title="Sphinx GPT Math Engine")
...
def home():
    return {
        "message": "Sphinx GPT Math Engine is running",
        "docs": "/docs",
        "endpoint": "/solve"
    }
    ...
    try:
            ...
            if "x^" in q:
                ...
                if "derivative" in q:
                    return {
                        ...
                        "engine": "Calculus",
                        "answer": f"{p}x^{p-1}",
                        "steps": f"Power Rule: d/dx(x^{p}) = {p}x^{p-1}"
                    }
                else:
                    return {
                        "success": True,
                        "engine": "Calculus",
                        "answer": f"(x^{p+1})/{p+1} + C",
                        "steps": f"Power Rule: ∫x^{p} dx = (x^{p+1})/{p+1} + C"
                    }
        ...
        if "die" in q or "coin" in q:
            return {
                "success": True,
                "engine": "Probability",
                "answer": "0.167" if "die" in q else "0.5"
            }
